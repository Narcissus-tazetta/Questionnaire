...
const ADMIN_ONLY = "🚫 このコマンドはサーバー管理者(サーバー管理権限)専用です。";
...
async function handleSetup(
  ...
  if (!drawTime || !roleId || !channelId) {
    return ephemeral("⚠️ `draw_time` / `role` / `channel` をすべて指定してください。");
  }
  if (!isValidDrawTime(drawTime)) {
    return ephemeral("⚠️ `draw_time` は `HH:MM`(24時間表記・日本時間)で指定してください。例: `20:00`");
  }
  ...
  logger.info("Guild configured", { guild: env.GUILD_ID, drawTime, roleId, channelId });
  ctx.waitUntil(
...
async function handleParticipants(interaction: Interaction, env: Env): Promise<Response> {
  ...
  if (!cfg) return ephemeral("⚠️ まだ `/setup` が実行されていません。");
  ...
  const list = people
    .map((p, i) => `${i + 1}. <@${p.userId}>${p.auto ? " 🔁" : ""}`)
    .join("\n");
  return ephemeral(
    `📋 ${date} の参加者(${people.length}名) 🔁=自動参加\n\n${list}`,
  );
}
...
function describeDraw(r: DrawResult, mode: "manual" | "reroll"): string {
  switch (r.status) {
    case "not_setup":
      return "⚠️ まだ `/setup` が実行されていません。";
    case "already_drawn":
      return r.winnerId
        ? `ℹ️ 本日は既に抽選済みです(当選者: <@${r.winnerId}>)。やり直すなら \`/reroll\` を使用してください。`
        : "ℹ️ 本日は既に処理済みです(参加者不在で担当継続)。";
    case "nothing_to_reroll":
      return "ℹ️ 本日はまだ抽選が行われていません。`/draw` を使用してください。";
    case "carryover":
      return r.winnerId
        ? `⚠️ 参加者がいなかったため、抽選は行われませんでした。<@${r.winnerId}> さんが引き続き担当です。`
        : "⚠️ 参加者がいなかったため、抽選は行われませんでした。前日の担当者もいないため、担当者は未定です。";
    case "drawn":
      return mode === "reroll"
